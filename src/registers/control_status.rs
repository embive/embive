...
use crate::{engine::EMBIVE_INTERRUPT_CODE, error::Error};
...
/// - MSTATUS (MIE, MPIE)
/// - MISA
/// - MIE (bit [`crate::engine::EMBIVE_INTERRUPT_CODE`])
/// - MTVEC (Direct mode only)
/// - MSCRATCH
/// - MEPC
/// - MCAUSE
/// - MIP (bit [`crate::engine::EMBIVE_INTERRUPT_CODE`])
///
...
pub struct CSRegisters {
    ...
    /// Machine Cause Register
    mcause: u32,
    /// Machine Interrupt Enable (bit [`crate::engine::EMBIVE_INTERRUPT_CODE`])
    mie_embive: bool,
    /// Machine Interrupt Pending (bit [`crate::engine::EMBIVE_INTERRUPT_CODE`])
    mip_embive: bool,
...
impl CSRegisters {
    ...
    /// Set `mip` bit [`crate::engine::EMBIVE_INTERRUPT_CODE`] to 1.
    ///
    /// Arguments:
    ...
    /// Returns true if `mie` bit [`crate::engine::EMBIVE_INTERRUPT_CODE`] and `mstatus.MIE` are set.
    #[inline(always)]
